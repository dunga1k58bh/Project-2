...
class Population():
    ...
    def __init__(self, pop_size, tasks):
        self.pop_size = pop_size
        self.__n_tasks = len(tasks)
        self.__tasks = tasks
        self.ranks = []
        ...
        for i in range(self.__n_tasks):
            if self.__tasks[i]._dimention > max:
                max = self.__tasks[i]._dimention
        self.__dimention = max
    ...
    def generate(self):
        ...
        for i in range(self.pop_size):
            ind = []
            for j in range(self.__dimention):
                ind.append(random())
    ...
    def checkInd(self, ind):
        for i in range(self.__n_tasks):
            if (self.__tasks[i].checkInd(ind) == False):
                return False
